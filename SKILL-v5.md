...
> 基於 naked-k-fib-bot v5 升級，新增安全基礎設施 + Momentum Long 策略 + OI 異常過濾

| name        | multi-strategy-trading-bot                                                                                                                                                                                                                                                                                                                                        |
| ----------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| description | 升級版幣安合約交易機器人，支援多策略切換：裸K+Fib、RSI均值回歸、Breakdown Short、Momentum Long。具備策略抽象基類、自動選幣、自動開單、風控、倉位同步、分批止盈、即時 Dashboard、Telegram 即時通知、API 重試容錯、每日虧損熔斷、OI 異常過濾。觸發時機：使用者提到「均值回歸」「RSI」「mean reversion」「多策略」「策略切換」「通知」「安全」「動量」「突破做多」。 |
...
├── 「動量突破做多」      → scripts/strategies/momentum_long.py
...
1. **v4 所有安全守則仍然適用**
2. 多策略同時跑時，總持倉不得超過 MAX_POSITIONS（預設 **6**），每筆保證金 ≈ 16.67%（`1/6`）平均分攝
...
| `scripts/strategies/momentum_long.py`  | Momentum Breakout Long 策略（牛市做多）    | P2     |
...
- RSI ≤ `MR_RSI_OVERSOLD`（25）+ 收盤 ≤ BB 下軌 × 1.005（0.5% 容忍）→ 做多
- RSI ≥ `MR_RSI_OVERBOUGHT`（75）+ 收盤 ≥ BB 上軌 × 0.995（0.5% 容忍）→ 做空
- ADX < 25（硬門檻，與選幣一致）
- 止跌/見頂 K 棒形態（`_has_reversal`）為入場必要條件
- 評分制（min_score=3，base score=1）：
    - 止跌/見頂 K 棒形態：+1
    - vol_ok（成交量 ≤ 均量 × `MR_VOL_MULT` = 1.2）：+1
    - RSI 極端（≤15 或 ≥85）：+1
    - BB 超出偏離（>10% BB 帶寬外）：+1
    - Stoch RSI 同向確認：+1
    - MACD 直方圖方向確認：+1
...
- TP1（**70%**）：min(1.2×ATR, 到BB中軌距離)（短距高勝率快出鎖利）
- TP2（**30%**）：BB 中軌（回歸目標）
- SL：`min(MR_SL_PCT × entry, 1 × ATR)`，限制在 `[0.5%, 3%]`
- R:R 門檻：`MR_MIN_RR = 1.05`（MR 專用，區別於其他策略的 1.2）
- 超時：`MR_TIMEOUT_BARS` 根 K 棒後強制平倉（預設 20 根 15m = 5 小時）
...
## Breakdown Short 策略速查（v5.2 新增）

### 選幣（`BreakdownShortStrategy._score_symbol`，滿分 10 分，≥ 6 才入選）

| 條件                                   | 分值                            | 實際判斷                               |
| -------------------------------------- | ------------------------------- | -------------------------------------- |
| 流動性：24h USDT 成交量 ≥ 500 萬       | +2（≥200萬得+1；不足直接跳過）  | `qav.tail(96).sum()`                   |
| 空頭結構：EMA20 < EMA50 + 價格 < EMA50 | +2（僅EMA交叉得+1；非空頭排除） | `ta.ema(close, 20/50)`                 |
| 趨勢強度：ADX 25-50                    | +2（20-25得+1；超出範圍排除）   | `adx_df["ADX_14"].iloc[-1]`            |
| 下降波段：近期 swing high 遞降         | +2（僅一個 swing得+1）          | `_find_swing_highs(left=5,right=5)`    |
| 波動適中：ATR 1.5%-8%                  | +2（1%-1.5%得+1）               | `atr / close * 100`                    |
| 過濾：近24h跳空 > 5% → 排除            | 硬門檻                          | `max(abs(open-prev_close)/prev_close)` |

**策略 ADX 分工**：NKF = ADX 15-45（有趨勢）；MR = ADX < 25（非趨勢）；BD = ADX 25-50（強趨勢做空）

### 入場（`BreakdownShortStrategy.check_signal`，只做空）

- 收盤跌破近 `BD_LOOKBACK_BARS`（20）根最低點（支撐突破）
- 跌破幅度 ≥ 0.1%（過濾假突破）
- 放量確認：成交量 ≥ `BD_VOL_MULT`（1.3）× 均量
- EMA20 < EMA50 + 價格 < EMA50（空頭結構）
- ADX > 20（非震盪）
- 評分制（min_score=3，base score=1）：
    - ADX > 30（強趨勢）：+1
    - 巨量突破（> 2x 均量）：+1
    - 空頭 K 棒形態（大陰線/射擊之星/空頭吞噬）：+1
    - BTC 週線空頭：+1
    - MACD 柱加速向下：+1

### 止盈止損

- TP1（60%）：Fib 1.272 extension（swing_low - diff × 0.272）
- TP2（40%）：Fib 1.618 extension（swing_low - diff × 0.618）
- SL：支撐位上方 + `BD_SL_ATR_MULT`（1.0）× ATR，限制在 [入場+0.3%, 入場+5%]
- 超時：`BD_TIMEOUT_BARS`（48）根 K 棒後強制平倉

---

## Momentum Breakout Long 策略速查（v5.3 新增）

### 選幣（`MomentumLongStrategy.screen_coins`，滿分 10 分，≥ 6 才入選）

| 條件                                   | 分值                            | 實際判斷                               |
| -------------------------------------- | ------------------------------- | -------------------------------------- |
| 流動性：24h USDT 成交量 ≥ 500 萬       | +2（≥200萬得+1；不足直接跳過）  | `qav.tail(96).sum()`                   |
| 多頭結構：EMA20 > EMA50 + 價格 > EMA50 | +2（僅EMA交叉得+1；非多頭排除） | `ta.ema(close, 20/50)`                 |
| 趨勢強度：ADX 25-50                    | +2（20-25得+1；超出範圍排除）   | `adx_df["ADX_14"].iloc[-1]`            |
| 上升波段：近期 swing low 遞升          | +2（僅一個 swing得+1）          | `_find_swing_lows(left=5,right=5)`     |
| 波動適中：ATR 1.5%-4%                  | +2（1%-1.5%得+1）               | `atr / close * 100`                    |
| 過濾：近24h跳空 > 5% → 排除            | 硬門檻                          | `max(abs(open-prev_close)/prev_close)` |

**策略 ADX 分工**：NKF = ADX 15-45；MR = ADX < 25；BD = ADX 25-50（強趨勢做空）；ML = ADX 25-50（強趨勢做多）

### 入場（`MomentumLongStrategy.check_signal`，只做多）

- 收盤突破近 `ML_LOOKBACK_BARS`（20）根最高點（阻力突破）
- 突破幅度 ≥ 0.1%（過濾假突破）
- 放量確認：成交量 ≥ `ML_VOL_MULT`（1.3）× 均量
- EMA20 > EMA50 + 價格 > EMA50（多頭結構）
- ADX > 20（非震盪）
- 評分制（min_score=3，base score=1）：
    - ADX > 30（強趨勢）：+1
    - 巨量突破（> 2x 均量）：+1
    - 多頭 K 棒形態（大陽線/錘子/多頭吞噬）：+1
    - BTC 週線多頭：+1
    - MACD 柱加速向上：+1

### 止盈止損

- TP1（60%）：Fib 1.272 extension（swing_high + diff × 0.272）
- TP2（40%）：Fib 1.618 extension（swing_high + diff × 0.618）
- SL：阻力位下方 - `ML_SL_ATR_MULT`（1.0）× ATR，限制在 [入場-5%, 入場-0.3%]
- 超時：`ML_TIMEOUT_BARS`（48）根 K 棒後強制平倉
...
Open Interest（未平倉合約量）24h 內劇烈變動 > `OI_CHANGE_MAX`%（預設 20%）時，代表大戶正在佈局或多空對決激烈，技術面分析容易失效。

### 實作

- `MarketContext.oi_change_pct(symbol)` — 查詢 Binance `futures_open_interest_hist` API（5m 粒度，288 根 = 24h），計算 `(oi_end - oi_start) / oi_start * 100`，結果有 TTL 快取
- `MarketContext.is_oi_anomaly(symbol, threshold_pct)` — `abs(oi_change) > threshold_pct` 時返回 True
- `CoinScreener.scan()` — 在評分前先跑 OI 異常過濾，跳過異常幣種

### 配置

```dotenv
OI_CHANGE_MAX=20.0    # OI 24h 變動 > 20% 視為異常，排除選幣
```
...
| 情境            | 啟動條件         | 追蹤距離                        |
| --------------- | ---------------- | ------------------------------- |
| NKF ADX > 35    | 開倉時即啟用     | `TRAILING_ATR_MULT`（1.5）× ATR |
| BD ADX > 35     | 開倉時即啟用     | 同上                            |
| ML ADX > 35     | 開倉時即啟用     | 同上                            |
| 所有策略 TP1 後 | TP1 成交自動啟用 | 同上（即時取得 ATR）            |

### 追蹤邏輯（`PositionSyncer._update_trailing`）

1. 每 30 秒同步時檢查
2. 更新 LONG 最高價 / SHORT 最低價
3. 啟動門檻：
    - `status=partial`（TP1 已成交）→ **立即追蹤**，不需等 1R
    - `status=open`（TP1 未成交）→ 需達 1R 獲利才啟動
4. 新止損 = 極值 ± `TRAILING_ATR_MULT` × ATR
5. 止損只能朝獲利方向推進，不可後退
...
| `scripts/strategies/__init__.py`       | 匯出 BaseStrategy / Signal / NKF / MR / BD / ML |
...
| 項目                                        | 狀態 | 說明                                                                            |
| ------------------------------------------- | ---- | ------------------------------------------------------------------------------- |
| `scripts/strategies/breakdown_short.py`     | ✅   | Breakdown Short 策略：熊市趨勢做空，支撐突破+放量+Fib extension TP              |
| 追蹤止盈增強                                | ✅   | TP1 後自動啟用 ATR 追蹤止盈（所有策略通用），`TRAILING_ACTIVATE_AFTER_TP1=true` |
| `position_syncer.py` 增強                   | ✅   | `_update_trailing()` v2：partial 狀態跳過 1R 門檻、可配置 `TRAILING_ATR_MULT`   |
| `state_manager.py` 新增 `enable_trailing()` | ✅   | 對已開倉交易動態啟用追蹤止盈                                                    |
| `bot_main.py` 多策略超時                    | ✅   | `check_strategy_timeout()` 統一處理 MR + BD + ML 超時平倉                       |
| `config.py` 新增參數                        | ✅   | `TRAILING_ATR_MULT`, `TRAILING_ACTIVATE_AFTER_TP1`, `BD_*` 系列參數             |

### v5.3 新增功能（已完成）

| 項目                                     | 狀態 | 說明                                                                                   |
| ---------------------------------------- | ---- | -------------------------------------------------------------------------------------- |
| NKF_TIMEFRAME 改 1h                      | ✅   | `config.py`：NKF_TIMEFRAME 預設值 "15m" → "1h"，減少雜訊                               |
| MR LONG BTC 週線過濾                     | ✅   | `mean_reversion.py`：LONG 信號先檢查 `btc_weekly_bullish()`，空頭時過濾                |
| HIGH_CORR_THRESHOLD 降為 0.6             | ✅   | `risk_manager.py`：相關性閾值 0.8 → 0.6，更早觸發分散風險                              |
| `scripts/strategies/momentum_long.py`    | ✅   | Momentum Breakout Long 策略：牛市趨勢做多，阻力突破+放量+Fib extension TP              |
| `BacktestMLEngine` + `run_backtest_ml()` | ✅   | `backtest.py`：ML 回測引擎，矢量化預計算指標，ADX/EMA/阻力突破邏輯                     |
| `bot_main.py` ML 整合                    | ✅   | ML 策略載入、超時配置、評分/R:R 映射、CLI 選擇                                         |
| `config.py` ML\_\* 參數                  | ✅   | ML_TIMEFRAME/ADX_MIN/ADX_MAX/LOOKBACK/VOL_MULT/SL_ATR/MIN_SCORE/TIMEOUT/MIN_RR/MAX_POS |
| OI 異常過濾                              | ✅   | `market_context.py` 新增 `oi_change_pct()` + `is_oi_anomaly()`                         |
| OI 整合選幣                              | ✅   | `coin_screener.py` scan 前檢查 OI 變動 > `OI_CHANGE_MAX`(20%) → 跳過                   |
| `config.py` OI_CHANGE_MAX                | ✅   | 預設 20.0，可透過 .env 調整                                                            |
...
| `GET /api/stats/all_strategies`  | ✅   | NKF + MR + BD + ML + 合併統計                                         |
| `POST /api/switch_strategy`      | ✅   | 熱切換 `ACTIVE_STRATEGY`（寫入 `.env`，下次排程生效）                 |
| `GET /api/active_strategy`       | ✅   | 查詢目前策略                                                          |
| strategy 欄位                    | ✅   | 持倉表 / 交易紀錄表新增策略欄，NKF=青色 / MR=紫色 / BD=紅色 / ML=綠色 |
| 策略分項統計 Panel               | ✅   | NKF / MR / BD / ML 並排卡片，每 60 秒自動刷新                         |
...
| `BacktestBDEngine`      | ✅   | BD 回測引擎（EMA/ADX/支撐突破/放量 on local DataFrame）                        |
| `run_backtest_bd()`     | ✅   | BD 主回測迴圈，矢量化預計算指標（O(n)），超時=`BD_TIMEOUT_BARS`                |
| `BacktestMLEngine`      | ✅   | ML 回測引擎（EMA/ADX/阻力突破/放量 on local DataFrame）                        |
| `run_backtest_ml()`     | ✅   | ML 主回測迴圈，矢量化預計算指標（O(n)），超時=`ML_TIMEOUT_BARS`                |
| `--strategy` 參數       | ✅   | `naked_k_fib` / `mean_reversion` / `breakdown_short` / `momentum_long` / `all` |
| `--adx-max` 參數        | ✅   | MR 回測 ADX 上限（預設 25，可改 20 更嚴格）                                    |
| `--scan` 模式           | ✅   | 批量掃描 20 個幣種，找最適合 MR 的幣，輸出排名表                               |
| 合併統計                | ✅   | `--strategy all` 時列印 NKF + MR + BD + ML 合計 + 四者合併                     |
...
# 只測 BD（自動讀取 BD_TIMEFRAME，預設 1h）
.venv\Scripts\python.exe scripts/backtest.py --strategy breakdown_short --symbol BTCUSDT --months 3

# 只測 ML（自動讀取 ML_TIMEFRAME，預設 1h）
.venv\Scripts\python.exe scripts/backtest.py --strategy momentum_long --symbol ETHUSDT --months 3
...
.venv\Scripts\python.exe scripts/bot_main.py --strategy breakdown_short
.venv\Scripts\python.exe scripts/bot_main.py --strategy momentum_long
...
```dotenv
# 策略選擇
ACTIVE_STRATEGY=all          # naked_k_fib / mean_reversion / breakdown_short / momentum_long / all

# MR 策略專屬
MR_TIMEFRAME=15m
MR_RSI_PERIOD=14
MR_RSI_OVERSOLD=25           # RSI 超賣門檻
MR_RSI_OVERBOUGHT=75         # RSI 超買門檻
MR_BB_PERIOD=20
MR_BB_STD=2.0
MR_VOL_MULT=1.2              # 成交量門檻（≤均量×1.2，用於評分bonus；非硬門檻）
MR_SL_PCT=0.025              # 止損百分比（最終SL=min(2.5%×entry, 1×ATR)，上限3%）
MR_MIN_SCORE=3               # 最低訊號評分（base=1，需至少2個bonus條件）
MR_MIN_RR=1.05               # MR 專用 R:R 門檻（低於其他策略的 1.2）
MR_TIMEOUT_BARS=20           # 超時平倉根數（15m × 20 = 5 小時）
MR_MAX_POSITIONS=3           # MR 最大持倉數

# 追蹤止盈（v5.2 新增）
TRAILING_ATR_MULT=1.5        # 追蹤距離 = N × ATR
TRAILING_ACTIVATE_AFTER_TP1=true  # TP1 後自動啟用追蹤止盈

# Breakdown Short 策略（v5.2 新增）
BD_TIMEFRAME=1h
BD_ADX_MIN=25                # 最低 ADX（需有趨勢）
BD_ADX_MAX=50                # 最高 ADX
BD_LOOKBACK_BARS=20          # 支撐突破回看根數
BD_VOL_MULT=1.3              # 突破量確認倍數
BD_SL_ATR_MULT=1.0           # SL = 突破點 + N×ATR
BD_MIN_SCORE=3               # 最低訊號評分
BD_TIMEOUT_BARS=48           # 超時平倉根數（1h × 48 = 2 天）
BD_MIN_RR=1.2                # 最低 R:R
BD_MAX_POSITIONS=2           # BD 最大持倉數
...
# Momentum Breakout Long 策略（v5.3 新增）
ML_TIMEFRAME=1h
ML_ADX_MIN=25                # 最低 ADX
ML_ADX_MAX=50                # 最高 ADX
ML_LOOKBACK_BARS=20          # 阻力突破回看根數
ML_VOL_MULT=1.3              # 突破量確認倍數
ML_SL_ATR_MULT=1.0           # SL = 突破點 - N×ATR
ML_MIN_SCORE=3               # 最低訊號評分
ML_TIMEOUT_BARS=48           # 超時平倉根數（1h × 48 = 2 天）
ML_MIN_RR=1.2                # 最低 R:R
ML_MAX_POSITIONS=2           # ML 最大持倉數

# OI 異常過濾（v5.3 新增）
OI_CHANGE_MAX=20.0           # OI 24h 變動 > 20% → 排除選幣
...
| 欄位           | 說明                                                                                         |
| -------------- | -------------------------------------------------------------------------------------------- |
| `strategy`     | "naked_k_fib" / "mean_reversion" / "breakdown_short" / "momentum_long"（預設 "naked_k_fib"） |
| `timeout_bars` | MR/BD/ML 超時計數器，每次 signal check 週期 +1                                               |
| `fib_level`    | NKF 用；MR 填 "—"；BD/ML 填 "—"                                                              |
| `pattern`      | NKF 用形態名；MR 填 "MR_REVERSAL"；BD 填 "BD_BREAKDOWN"；ML 填 "ML_BREAKOUT"                 |
...
**Q: MR 超時後沒有平倉？**
A: 確認 `bot_main.py` 的 `check_strategy_timeout()` 有在排程中，`MR_TIMEOUT_BARS` 是否設定正確。

**Q: 多個策略搶同一個幣？**
A: 選幣邏輯已分離：MR 只選 ADX<25；NKF 只選 ADX 15–45；BD/ML 只選 ADX 25–50（BD 做空、ML 做多），避免重疊。
